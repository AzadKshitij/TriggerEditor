...
class LoggingDock(QDockWidget):
    ...
    def log(self, level: str, message: str, design_window_id: Optional[str] = None) -> None:
        ...
        if len(self.logs_per_window[design_window_id]) > self.max_logs:
            self.logs_per_window[design_window_id] = self.logs_per_window[design_window_id][-self.max_logs:]
    ...
    def apply_filters(self) -> None:
        ...
        current_logs = []
        if self.current_design_window_id and self.current_design_window_id in self.logs_per_window:
            current_logs = self.logs_per_window[self.current_design_window_id]
        
        # Get level priority for filtering
        level_priorities = {level["name"]: i for i, level in enumerate(LogLevel.ALL_LEVELS)}
