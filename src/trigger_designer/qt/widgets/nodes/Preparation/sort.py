...
class SortContent(QDMNodeIconContentWidget, TriggerChangeHandler):
    ...
    def store_history(self, old_state: Dict[str, Any], new_state: Dict[str, Any]) -> None:
        """
    ...
    def get_code(self) -> str:
        ...
        if len(sort_conditions) == 1:
            ...
            code_lines.append(f"{self.variable_name} = {self.incoming_variable}.sort('{column}', descending={descending})")
        else:
            # Multiple column sort
            columns = [f'{col}' for col, _ in sort_conditions]
            descending_flags = [desc for _, desc in sort_conditions]
            code_lines.append(f"{self.variable_name} = {self.incoming_variable}.sort({columns}, descending={descending_flags})")
