...
class CountRecordsContent(QDMNodeIconContentWidget, TriggerChangeHandler):
    ...
    def __init__(self, node: 'TriggerNode', parent: Optional[QWidget] = None) -> None:
        ...
        global_logger.debug("🔢 CountRecordsContent: Initializing Count Records node content widget")
        self.node = node
    ...
    def deserialize(self, data, hashmap={}):
        ...
        try:
            global_logger.debug("🔢 CountRecordsContent: Deserializing Count Records node")
            ...
            global_logger.error(f"❌ CountRecordsContent: Deserialization failed: {str(e)}")
        return res

    def process_data(self) -> None:
        """Count total records in DataFrame using Polars"""
        if self.incom_data is not None:
            global_logger.debug("🔢 CountRecordsContent: Processing data to count records")
            try:
                ...
                global_logger.info(f"✅ CountRecordsContent: Successfully counted {self.total_records:,} records")
            except Exception as e:
                global_logger.error(f"❌ CountRecordsContent: Error counting records: {str(e)}")
                self.total_records = 0
                self.data = None
    ...
    def get_code(self) -> str:
        ...
        code_lines.append(f"{self.variable_name} = pl.DataFrame({{'Count': [{self.incoming_variable}.shape[0]]}})")
...
class TriggerNode_CountRecords(TriggerNode):
    ...
    def __init__(self, scene) -> None:
        ...
        global_logger.debug("🔢 CountRecordsNode: Node created with 1 input and 1 output")
        self.eval()
    ...
    def processInputs(self, input_values):
        ...
        try:
            ...
            global_logger.debug(f"🔢 CountRecordsNode: Retrieved input data, socket index: {socket_index}")

            if input_value:
                global_logger.info("✅ CountRecordsNode: Input data received, processing...")
                print("We have input")
                ...
                if input_data is not None:
                    global_logger.info(f"🔢 CountRecordsNode: Processing DataFrame with shape {input_data.shape} for variable '{variable_name}'")
                    ...
                    if hasattr(self.content, 'data') and self.content.data is not None:
                        ...
                        global_logger.info(f"🔢 CountRecordsNode: Output DataFrame shape: {output_shape}")
                        global_logger.info(f"🔢 CountRecordsNode: Total records counted: {self.content.total_records:,}")
                        ...
                        self.evalChildren()
                        global_logger.info("✅ CountRecordsNode: Processing completed successfully")
                        return self.param
                    else:
                        global_logger.error("❌ CountRecordsNode: No output data generated after processing")
                        self.markDirty(True)
                        self.markInvalid(True)
                        return None
                else:
                    global_logger.warning("⚠️ CountRecordsNode: Input value contains no data")
                    self.markDirty(True)
                    self.markInvalid(True)
                    return None
            ...
            global_logger.error(f"❌ CountRecordsNode: Error during input processing: {str(e)}")
            global_logger.critical(f"🚨 CountRecordsNode: Exception details: {type(e).__name__}: {str(e)}")
            self.markDirty(True)
            self.markInvalid(True)
