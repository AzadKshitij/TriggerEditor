...
class TransposeContent(QDMNodeIconContentWidget, TriggerChangeHandler):
    ...
    def __init__(self, node: 'TriggerNode', parent: Optional[QWidget] = None) -> None:
        ...
        global_logger.debug("📊 TransposeContent: Initializing Transpose node content widget")
    ...
    def create_layout(self, dock_layout: QVBoxLayout) -> None:
        if self.incom_data is not None:
            ...
            key_info = QLabel("These columns will be preserved as identifiers in the transposed result.")
            ...
            data_info = QLabel("These columns will be transposed from columns to rows. Column names become 'Name' values, column data becomes 'Value' values.")
            ...
            action_info = QLabel("How to handle columns that are selected but don't exist in the input data.")
    ...
    def process_data(self) -> None:
        ...
        if self.incom_data is not None and self.data_columns:
            global_logger.debug("📊 TransposeContent: Processing data for transpose operation")
            try:
                available_columns = self.incom_data.columns
                
                # Validate columns exist
                missing = [col for col in self.data_columns if col not in available_columns]
                if missing:
                    if self.missing_action == "error":
                        global_logger.error(f"❌ TransposeContent: Missing columns: {missing}")
                        raise ValueError(f"Missing columns: {missing}")
                    elif self.missing_action == "warn":
                        global_logger.warning(f"⚠️ TransposeContent: Missing columns will be skipped: {missing}")
                        print(f"Warning: Missing columns will be skipped: {missing}")

                # Validate key columns exist
                missing_key_cols = [col for col in self.key_columns if col not in available_columns]
                if missing_key_cols:
                    global_logger.warning(f"⚠️ TransposeContent: Missing key columns will be skipped: {missing_key_cols}")
                    
                # Filter to existing columns only
                valid_key_cols = [col for col in self.key_columns if col in available_columns]
                valid_data_cols = [col for col in self.data_columns if col in available_columns]
                
                if not valid_data_cols:
                    global_logger.warning("⚠️ TransposeContent: No valid data columns found for transpose")
                    self.data = None
                    return

                global_logger.debug(f"📊 TransposeContent: Index columns: {valid_key_cols}")
                global_logger.debug(f"📊 TransposeContent: Value columns: {valid_data_cols}")
                ...
                global_logger.info(f"✅ TransposeContent: Transpose completed - Result shape: {self.data.shape}")

            except Exception as e:
                global_logger.error(f"❌ TransposeContent: Error in transpose operation: {str(e)}")
    ...
    def on_key_selection_changed(self) -> None:
        ...
        global_logger.debug(f"📊 TransposeContent: Key columns updated: {self.key_columns}")
        self.process_data()
        self.evaluate.emit()
    ...
    def on_data_selection_changed(self) -> None:
        ...
        global_logger.debug(f"📊 TransposeContent: Data columns updated: {self.data_columns}")
        self.process_data()
        self.evaluate.emit()
    ...
    def on_action_changed(self, value: str) -> None:
        ...
        global_logger.debug(f"📊 TransposeContent: Missing action changed to: {self.missing_action}")
        self.process_data()
        self.evaluate.emit()
    ...
    def deserialize(self, data: dict, hashmap={}) -> bool:
        ...
        try:
            ...
            global_logger.debug(f"📊 TransposeContent: Restored key columns: {self.key_columns}")
            global_logger.debug(f"📊 TransposeContent: Restored data columns: {self.data_columns}")
            global_logger.debug(f"📊 TransposeContent: Restored missing action: {self.missing_action}")

            return True & res
        except Exception as e:
            global_logger.error(f"❌ TransposeContent: Deserialization failed: {str(e)}")
            dumpException(e)
            return res
...
class TriggerNode_Transpose(TriggerNode):
    ...
    def processInputs(self, input_values):
        ...
        try:
            ...
            global_logger.debug(f"📊 TransposeNode: Retrieved input data, socket index: {socket_index}")

            if input_value:
                global_logger.info("✅ TransposeNode: Input data received, processing...")
                print("We have input")
                ...
                if input_data is not None:
                    global_logger.info(f"📊 TransposeNode: Processing DataFrame with shape {input_data.shape} for variable '{variable_name}'")
                    ...
                    if hasattr(self.content, 'data') and self.content.data is not None:
                        ...
                        global_logger.info(f"📊 TransposeNode: Output DataFrame shape: {output_shape}")
                        ...
                        self.evalChildren()
                        global_logger.info("✅ TransposeNode: Processing completed successfully")
                        return self.param
                    else:
                        global_logger.error("❌ TransposeNode: No output data generated after processing")
                        self.markDirty(True)
                        self.markInvalid(True)
                        return None
                else:
                    global_logger.warning("⚠️ TransposeNode: Input value contains no data")
                    self.markDirty(True)
                    self.markInvalid(True)
                    return None
            ...
            global_logger.error(f"❌ TransposeNode: Error during input processing: {str(e)}")
            global_logger.critical(f"🚨 TransposeNode: Exception details: {type(e).__name__}: {str(e)}")
            self.markDirty(True)
            self.markInvalid(True)
