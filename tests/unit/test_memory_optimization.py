...
"""
    ...
    for i in range(1000):  # Create ~4000 rows
        large_data += (
            sample_data.replace("P001", f"P{i:04d}_1")
            .replace("P002", f"P{i:04d}_2")
            .replace("P003", f"P{i:04d}_3")
            .replace("P004", f"P{i:04d}_4")
        )
    ...
    try:
        ...
        df_preview = pl.read_csv(
            temp_file, n_rows=5, infer_schema=False, low_memory=True, rechunk=False
        )
        print(
        ...
        print(f"Full file loaded: {df_full.shape[0]} rows, {df_full.shape[1]} columns")
        ...
        print(
            "The optimizations should show significantly lower memory usage for preview operations."
        )
