...
from qtpy.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QHBoxLayout, QPushButton, QLabel
...
sys.path.append(
    r'c:\Users\KASHVINCHANDRASAN\Desktop\Personal\Github\TriggerEditor\src')

try:
    from trigger_designer.qt.models.polars_table_viewer import PolarsTableViewer
    from trigger_designer.qt.models.enhanced_data_preview_window import EnhancedDataPreviewWindow, preview_data
...
def generate_sample_data(rows: int = 10000, include_nulls: bool = True) -> pl.DataFrame:
    ...
    data = {
        ...
        'name': [''.join(random.choices(string.ascii_letters, k=random.randint(5, 15)))
                 for _ in range(rows)],
        'description': ['Description for item ' + str(i) + ' with random text: ' +
                        ''.join(random.choices(string.ascii_letters +
                                ' ', k=random.randint(20, 100)))
                        for i in range(rows)],
        'category': np.random.choice(['Electronics', 'Clothing', 'Books', 'Sports', 'Home'], rows),
    ...
    if include_nulls:
        ...
        df = df.with_columns([
            pl.when(null_mask_price).then(None).otherwise(
                pl.col('price')).alias('price'),
            pl.when(null_mask_rating).then(None).otherwise(
                pl.col('rating')).alias('rating'),
            pl.when(null_mask_description).then(None).otherwise(
...
class PolarsViewerDemo(QMainWindow):
    ...
    def _open_preview_window(self):
        ...
        preview_window = preview_data(
            self.current_df, "Polars DataFrame Preview")
